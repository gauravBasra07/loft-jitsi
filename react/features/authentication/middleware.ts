...
import {
    ...
    CONNECTION_FAILED,
} from "../base/connection/actionTypes";
    ...
    switch (action.type) {
        ...
        case CONFERENCE_FAILED: {
            ...
            if (
                ...
                (error.name === JitsiConferenceErrors.MEMBERS_ONLY_ERROR &&
                    lobbyWaitingForHost)
            ) {
        ...
        case CONFERENCE_JOINED: {
            ...
            let url: any = state["features/base/connection"]?.locationURL?.href;
            sessionStorage.setItem("locationURL", url);

            let origin: any =
                state["features/base/connection"]?.locationURL?.origin;
            sessionStorage.setItem("origin", origin);

            let meetingId: any = state["features/base/conference"]?.room;
            sessionStorage.setItem("meetingId", meetingId);

            let userData: any = state["features/base/jwt"].user;
        ...
        case WAIT_FOR_OWNER: {
            ...
            const {
                handler,
                timeoutMs,
            }: { handler: () => void; timeoutMs: number } = action;
...
function _handleLogin({ dispatch, getState }: IStore) {
    ...
    getTokenAuthUrl(config, room, tenant, true, locationURL).then(
        (tokenAuthServiceUrl: string | undefined) => {
            if (!tokenAuthServiceUrl) {
                logger.warn(
                    "Cannot handle login, token service URL is not set"
                );
